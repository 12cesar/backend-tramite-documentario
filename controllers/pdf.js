...
const pdfDocInter = async (req = request, res = response) => {
  ...
  let ubicacion = path.join(__dirname,'../document/','pdf',`documento-interno-${idArea}.pdf`);
  pdf.create(html, options).toFile(ubicacion, function (err, resp) {
    ...
    res.status(400).json({
      ok: false,
      msg: `Error al crear el documento interno, ${error}`,
    });
  }
};
...
const getDocumentoInternoPdf = (req = request, res = response) => {
  ...
  doc.setDocumentHeader({}, () => {
    doc
      .lineJoin("miter")
      .rect(0, 0, 100, doc.header.options.heightNumber)
      .fill("#fff");
    ...
    doc
      ...
      .fontSize(9)
      .text(
        "Oficina Ejecutiva de Planeamiento, Finanzas Gestion Institucional",
        doc.header.x,
        doc.header.y + 30,
        {
          align: "center",
